...
//   // 对响应数据做点什么
//   var cvs = store.state.config.configVersion;
//   if (cvs && cvs == response.data.cvs) {
...
// 将请求的数据拼接成字符串形式
function transformRequestData(obj) {
    var queryData = '';
    ...
    for(name in obj) {
        ...
        if (value instanceof Array) {
            for(var i = 0; i < value.length; i++) {
                ...
                queryData += transformRequestData(innerObj) + '&';
            }
            ...
            queryData += encodeURIComponent(name) + '=' + encodeURIComponent(value) + '&';
        }
    }
